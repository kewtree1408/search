...
def finder(rindex, query):
    ...
    freq_docids = sorted([rindex[t] for t in terms_and_query], key=lambda freq: freq[0])
...
def main():
    ...
    docids = sorted(list(finder(rindex, u_query)))
    print "Query in ", docids

    # выводим результаты: номер параграфа и сниппет с искомыми словами
    for docid in docids[:nparagraph]:
        snippet = get_snippet(u_query, doc_context[docid], nsnippet)
        print "%d: " % docid
        for s in snippet:
            print "\t", light_text(get_query_terms(u_query), s)
